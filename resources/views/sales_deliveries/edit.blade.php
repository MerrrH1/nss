        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <h3 class="text-lg font-medium mb-6">Edit Unit Pengiriman: {{ $salesDelivery->delivery_number }}</h3>
                    ...
                    <form method="POST" action="{{ route('sales_deliveries.update', $salesDelivery) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            {{-- Sales Contract Info (Read-only) - Ambil dari $salesDelivery->salesContract --}}
                            <div class="md:col-span-2 p-4 bg-gray-100 dark:bg-gray-700 rounded-md">
                                ...
                                <p class="text-sm text-gray-900 dark:text-gray-200">
                                    Nomor Kontrak: <span class="font-bold">{{ $salesDelivery->salesContract->contract_number }}</span>
                                </p>
                                <p class="text-sm text-gray-900 dark:text-gray-200">
                                    Total Kuantitas: <span class="font-bold">{{ number_format($salesDelivery->salesContract->total_quantity_kg, 0, ',', '.') }} Kg</span>
                                </p>
                                <p class="text-sm text-gray-900 dark:text-gray-200">
                                    Kuantitas Terkirim Saat Ini: <span class="font-bold">{{ number_format($salesDelivery->salesContract->quantity_delivered_kg, 0, ',', '.') }} Kg</span>
                                </p>
                                <p class="text-sm text-gray-900 dark:text-gray-200">
                                    Sisa Kuantitas Tersedia: <span class="font-bold text-blue-600" id="remaining_contract_quantity">{{ number_format($salesDelivery->salesContract->total_quantity_kg - $salesDelivery->salesContract->quantity_delivered_kg, 0, ',', '.') }} Kg</span>
                                </p>
                                {{-- Hidden input ini mungkin tidak perlu di halaman edit jika sales_contract_id sudah dipertahankan oleh $salesDelivery --}}
                                <input type="hidden" name="sales_contract_id" id="sales_contract_id" value="{{ $salesDelivery->sales_contract_id }}">
                            </div>
                            ...
                            <div>
                                ...
                                <select name="truck_id" id="truck_id"
                                    ...
                                    @foreach ($trucks as $truck)
                                        <option value="{{ $truck->id }}" {{ old('truck_id', $salesDelivery->truck_id) == $truck->id ? 'selected' : '' }}>
                                            {{ $truck->plate_number }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <input type="number" name="gross_weight_kg" id="gross_weight_kg" step="0.01"
                                    value="{{ (int) old('gross_weight_kg', $salesDelivery->gross_weight_kg) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200" required>
                            ...
                            <div>
                                ...
                                <input type="number" name="tare_weight_kg" id="tare_weight_kg" step="0.01"
                                    value="{{ (int) old('tare_weight_kg', $salesDelivery->tare_weight_kg) }}"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-200" required>
                            ...
                            <div>
                                ...
                                <input type="text" id="net_weight_kg_display"
                                    ...
                                    value="{{ (int) old('net_weight_kg', $salesDelivery->net_weight_kg) }}" readonly>
                                <input type="hidden" name="net_weight_kg" id="net_weight_kg" value="{{ old('net_weight_kg', $salesDelivery->net_weight_kg) }}">
                                @error('net_weight_kg')
                                    <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            {{-- Gross Weight --}}
                            <div>
                                <label for="final_gross_weight_kg" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Berat Bruto Final (Kg)</label>
                                <input type="number" name="final_gross_weight_kg" id="final_gross_weight_kg" step="0.01"
                            ...
                            {{-- Tare Weight --}}
                            <div>
                                <label for="final_tare_weight_kg" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Berat Tara Final (Kg)</label>
                                <input type="number" name="final_tare_weight_kg" id="final_tare_weight_kg" step="0.01"
                            ...
                            <div>
                                ...
                                <input type="text" id="final_net_weight_kg_display"
                                    class="mt-1 block w-full rounded-md border-gray-300 shadow-sm bg-gray-100 dark:bg-gray-700 dark:border-gray-600 dark:text-gray-400 cursor-not-allowed"
                                    value="{{ (int) old('final_net_weight_kg', $salesDelivery->final_net_weight_kg) }}" readonly>
                                <input type="hidden" name="final_net_weight_kg" id="final_net_weight_kg" value="{{ old('final_net_weight_kg', $salesDelivery->final_net_weight_kg) }}">
                            ...
                            <div>
                                <label for="dobi" class="block text-sm font-medium text-gray-700 dark:text-gray-300">DOBI (%)</label>
