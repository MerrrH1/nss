<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-, $supplier->xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Edit Penjual') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-8xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <h3 class="text-lg font-medium mb-6">Edit Penjual:
                    </h3>
                    @if ($errors->any())
                    ...
                    <form method="POST" action="{{ route('suppliers.update', $supplier->id) }}">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="email" id="email" value="{{ old('email', $supplier->email) }}"
                        ...
                        <div class="mt-6 flex justify-end">
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-, $supplier->xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                Perbarui Kontrak
                            </button>
                            <a href="{{ route('suppliers.show', $supplier->id) }}"
                                class="ml-4 inline-flex items-center px-4 py-2 bg-gray-500 border border-transparent rounded-md font-semibold text-, $supplier->xs text-white uppercase tracking-widest hover:bg-gray-600 focus:bg-gray-600 active:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                Batal
                            </a>
                        </div>
                    </form>
